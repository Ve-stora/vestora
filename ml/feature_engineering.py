...
Designed for thin EAC markets — handles sparse data, zero-volume days, and
low-liquidity counters that standard TA libraries choke on.
"""
...
def add_rolling_stats(df: pd.DataFrame) -> pd.DataFrame:
    """MA and volatility over multiple windows."""
    for w in [5, 10, 20, 30]:
        df[f"ma{w}"]   = df["close"].rolling(w, min_periods=max(1, w // 2)).mean()
        df[f"std{w}"]  = df["close"].rolling(w, min_periods=max(1, w // 2)).std().fillna(0)
    return df
...
def add_returns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Forward returns (targets — shift -1 so today's row has tomorrow's return)
...
def add_macd(df: pd.DataFrame) -> pd.DataFrame:
    """MACD adapted for low-liquidity — use shorter periods for thin markets."""
    ema12 = df["close"].ewm(span=12, min_periods=6,  adjust=False).mean()
    ema26 = df["close"].ewm(span=26, min_periods=13, adjust=False).mean()
    df["macd"]        = ema12 - ema26
...
def add_ma_ratios(df: pd.DataFrame) -> pd.DataFrame:
    """Price relative to moving averages — strong features for momentum."""
    for w in [5, 10, 20]:
        ma_col = f"ma{w}"
...
def add_volume_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Volume features.
    Handles zero-volume days (no trades) explicitly — crucial for USE counters.
    """
...
def add_price_gap(df: pd.DataFrame) -> pd.DataFrame:
    """Overnight gap — requires open price. Safe fallback if missing."""
    if "open" in df.columns:
        prev_close = df["close"].shift(1)
        df["price_gap"] = (
            (df["open"] - prev_close).abs() / prev_close.replace(0, np.nan)
        ).fillna(0).clip(0, 0.5)
    else:
        df["price_gap"] = 0.0
    return df
...
def add_price_position(df: pd.DataFrame) -> pd.DataFrame:
    """Where current price sits within 52-week range — useful momentum context."""
